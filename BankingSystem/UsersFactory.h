...
User* factory(const char* firstName, const char* secondName, const char* nameOfPossition, unsigned IDNumber, unsigned age, const char* password) {
	if (strcmp(nameOfPossition, "Client") == 0) {
		return new Client(firstName, secondName, IDNumber, age, password);
	}
	else if (strcmp(nameOfPossition, "Employee") == 0) {
		return new Employee(firstName, secondName, IDNumber, age, password);
	}
