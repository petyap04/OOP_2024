...
void ChangeAccountCommand::execute()
{
	try {
		...
		Task* task = new Change_task(banking_system.getCurrentUser(),accountNumber, newBank);
		banking_system.getBankOnIndex(ind1).giveTaskToTheEmployeeWithTheLeastTasks(task);
	}
